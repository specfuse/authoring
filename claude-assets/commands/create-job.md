...
*Enforces: handbooks/AsyncAPI_Handbook.md*

**Before writing any files**, read and internalize the authoritative rules:
1. Read `handbooks/AsyncAPI_Handbook.md` — Sections 0.4, 1.3, 4.3, 5.2
2. Read `api/specs/CLAUDE.md` — Section 10 (AsyncAPI Files — Critical Rules)
3. Read `samples/message-samples.yaml` — samples 2 (scheduled job message), 4 (scheduled-trigger channel), 6 (job operation)
...
If the per-item event is a state transition (e.g. `*ArchiveRequested` is a transition action), add `x-trigger-when` and follow the transition payload shape with both `before` and `after`. See `samples/message-samples.yaml` §1c.

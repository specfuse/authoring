...
6. `.claude/agents/handoff-composer.md` — the composer agent's input/output contract.
...
Spawn the `handoff-composer` agent with the full structured brief assembled from Steps 1–10. The agent reads `.claude/agents/handoff-composer.md` for its input/output contract, applies HC-1 through HC-8, returns the manifest as Markdown.

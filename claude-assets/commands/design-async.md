...
1. Read `handbooks/AsyncAPI_Handbook.md` — the full handbook, especially Section 1 (Architectural Principles)
2. Read `api/specs/CLAUDE.md` — Section 10 (AsyncAPI Files — Critical Rules)
3. Read `samples/message-samples.yaml` — canonical templates to copy from
...
Snapshots live in `domains/{domain}/events/{Entity}Snapshot.yaml` — one per event-emitting entity, reused across all of its events. See `samples/message-samples.yaml` §1 for canonical examples of every action class.

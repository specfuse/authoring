...
**Authoritative rules:** `handbooks/Arazzo_Handbook.md` -- if anything in this file contradicts the handbook, the handbook wins.
...
| Source | Path | Purpose |
|---|---|---|
| Arazzo Handbook | `handbooks/Arazzo_Handbook.md` | Authoritative rules for all extensions and patterns |
| Scenario samples | `samples/scenario-samples.yaml` | Canonical template -- match this structure |
| Recipe samples | `samples/recipe-samples.yaml` | Understand recipe composition and output contracts |
...
Reference implementation: see the AI-worker pattern in `samples/scenario-samples.yaml`.
...
Reference implementation: see the scheduled-job pattern in `samples/scenario-samples.yaml`.
...
The generated YAML should match the quality level of the canonical template in `samples/scenario-samples.yaml`, which covers:

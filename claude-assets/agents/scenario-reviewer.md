...
**Authoritative rules:** `handbooks/Arazzo_Handbook.md` -- especially Section 2 (granularity), Section 13 (Do/Don't).
...
| Source | Path | Purpose |
|---|---|---|
| Arazzo Handbook | `handbooks/Arazzo_Handbook.md` | Authoritative rules -- the standard against which everything is measured |
| Scenario samples | `samples/scenario-samples.yaml` | Canonical template -- conformance benchmark |
| Recipe samples | `samples/recipe-samples.yaml` | Recipe conventions (when reviewing recipes) |
...
The reviewer MUST produce **APPROVE** for the canonical pilot scenarios shipped with `samples/scenario-samples.yaml` and `samples/recipe-samples.yaml` (possibly with minor `[info]` notes). These cover:

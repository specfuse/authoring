...
| `v0.11.0` (incubating, current) | generator **`0.6.0`** — `Specfuse/generator-dist` release `v0.6.0`, asset `specfuse-generator-0.6.0.jar`, sha256 `e821f045…e658` | **Kit release — the pinned generator is BREAKING in two independent ways, and this is the first pin bump where something that validated clean before does not after.** *Minor, not patch, for that reason alone.* **(1) `PROTECTION_ATREST_REQUIRED` is a new ERROR** (generator FEAT-2026-0119 gate 1, the Law 25 / GDPR / PIPEDA / SOC 2 field-protection vocabulary). A property carrying `x-classification` on a schema with `x-entity` must now also declare `x-protection.atRest`. Measured on `examples/hello-orders`: **0.5.8 gave PASSED / 0 ERROR / 20 WARNING / 4 SUGGESTION; 0.6.0 gave FAILED / 3 ERROR / 21 WARNING / 4 SUGGESTION** — the three errors all on `Customer`'s `firstName`, `lastName` and `email`, the example's only classified properties. **The bundled example is fixed in this same release** and now reports PASSED / 0 ERROR on 0.6.0, *and still PASSED / 0 ERROR on 0.5.8* — `x-protection` is additive and the older jar ignores it, so a spec can adopt the vocabulary before its consumers take the pin. Copy `Customer.yaml` as the migration reference: `atRest: none` with a `rationale`, a `reviewedOn` and a `reviewedBy` is the expected outcome for ordinary PII, and the point is that the decision is now recorded rather than assumed. **(2) Generated C# application-service bases take a new constructor parameter.** `IConcurrencyTokenSink` is appended to the protected constructor of **every** emitted `{Tag}ApplicationServiceBase` — 6 of 6 on the reference bundle, including bases that publish no token today — so every consumer subclass's `base(…)` call fails with **CS7036** until it forwards the new argument. Three application-routed reads also move from the interface onto a non-virtual base wrapper plus an abstract `…Core`, giving **CS0108 / CS0534** to a consumer that had already adopted the base. Both are deliberate: the alternative emits the parameter only where a token-publishing read exists today and changes the other bases one gate later, which is two migrations for one contract. **⊕ Also in 0.6.0, no kit action:** an unknown artifact name in `project.json` is now rejected at load time instead of after `cleanGenerated` has emptied the destination (generator #1230 — it wiped 256 tracked files in a consumer repo before the fix); generated C# comments stop citing the generator's own issue tracker and authoring rules (#1237); and `SERVICE_REGISTRY_MISSING` appears as one new WARNING on any bundle with no `info.x-services`, which is the +1 in the warning counts above. **Spec-author action:** declare `x-protection` on every classified entity property before taking this pin — `validate` names each one — and forward `IConcurrencyTokenSink` in every application-service subclass constructor. |

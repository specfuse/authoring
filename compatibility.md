...
| `v0.5.6` (incubating, current) | generator **`0.5.4`** (pin unchanged) | **Kit-only release — project upgrade path.** Before this, a kit release could not reach a project that already existed: `init` copied the template once at creation and nothing re-delivered it, while `refresh` blind-copied only handbooks and samples with no record of what it had written. New **`specfuse-authoring upgrade <project>`** (with `--dry-run`) overlays the kit-owned files — handbooks, samples, **schemas** (newly delivered; projects previously never received the Spectral rulesets they were told to lint against) and the `scripts/` tooling the plugin skills invoke. Mirrors the scaffold overlay in `specfuse/loop`: a `.specfuse/authoring/VERSION` stamp that refuses a downgrade, a sha256 `.scaffold-manifest` ownership record, clobber warnings that distinguish a locally-modified kit file from a project-authored one, and a manifest-scoped prune that deletes only what the kit provably wrote. Projects predating the overlay are adopted on first run (`kit unversioned -> …`). Project content — `api/`, `CLAUDE.md`, the project file, `.gitignore` — is seeded once at `init` and never touched again. `refresh` remains as a deprecated alias. **Spec-author action:** run `specfuse-authoring upgrade <project>` after each kit release; edits to shipped kit files do not survive it (the warning names them), so send improvements upstream instead. |

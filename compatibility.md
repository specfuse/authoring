...
| `v0.5.9` (incubating) | generator **`0.5.5`** (pin unchanged) | **Kit-only release — `examples/hello-orders/` now passes generator validation (authoring #33), and the `x-default` guidance is corrected.** The example carried six generator errors, invisible because CI cannot run the generator. **(1) The `Tenant` aggregate was missing.** `Customer` and `Order` both declared `belongsTo: [Tenant]` and every resource route is scoped by `/v1/tenants/{tenantId}`, but no schema with `x-entity` existed for it — so the example demonstrated multi-tenancy with the tenancy root absent (`INVALID_RELATIONSHIP_TARGET` ×2, `OPERATION_PARAM_ENTITY_NOT_FOUND` ×2). Added as an aggregate with no `belongsTo` (the tenancy root is the one legitimate case for omitting it), a new `tenant` domain in `info.x-domains`, and a Tier 0 `aiAccess` block — tenancy is an infrastructure boundary, not agent-selectable data. **(2) Required state-machine enums had no default** (`REQUIRED_ENUM_MISSING_DEFAULT` ×2): an entity with a required enum property and no default has no defined state at creation. `CustomerStatus` now defaults to `active`, `OrderStatus` to `draft`. **(3) `Vendor_Extensions.md` §4.6 corrected.** It advised preferring the standard OpenAPI `default` and reaching for `x-default` only where `default` falls short. On an enum schema that is backwards: the generator errors (`ENUM_MISSING_X_DEFAULT`) unless **both** are present with the same value, because OpenAPI Generator does not reliably pick up `default` alone. The section now states that, adds the `REQUIRED_ENUM_MISSING_DEFAULT` rule, and notes the default belongs on the enum schema rather than beside the `$ref` — OpenAPI 3.0 silently drops keywords declared as siblings of `$ref`. **Spec-author action:** pair every enum `default` with a matching `x-default`, and give required state-machine enums one. |

| `v0.5.10` (incubating, current) | generator **`0.5.5`** (pin unchanged) | **Kit-only release — `validate-openapi-generator.sh` selects specs by content (authoring #38).** The script globbed `*.yaml` at the spec root, and the documented layout puts `asyncapi.yaml` beside `openapi.yaml` — so an AsyncAPI 3.0 document was handed to `openapi-generator-cli validate`, which rejects it by construction. The layer could never pass in any project with async specs, and `validate-specs.sh` aggregates it, so the full suite could never go green either. OpenAPI documents are now identified by a top-level `openapi:` key, and skipped files are reported rather than silently failing. Also fixes an unparenthesised `-o` in the same `find` that let `-maxdepth 1` apply only to the first term, so `*.yml` and `*.json` matched at any depth. CI asserts both, and the assertions fire on a simulated regression. **Spec-author action:** `specfuse-authoring upgrade <project>`. |

...
| `v0.5.5` (incubating) | generator **`0.5.4`** (pin unchanged) | **Kit-only release — no generator coordination required.** Ruleset and documentation fixes from consumer reports #12–#15. (1) **`x-entity.mutability`** (NEW optional sub-field, authoring #13) — `mutable` | `immutable` | `appendOnly`, default `mutable`. `specfuse-main-resource-has-updatedAt` now exempts entities declaring a non-mutable write policy, instead of demanding an `updatedAt` that would permanently equal `createdAt` on audit-trail, ledger and append-only entities. The value is validated against the closed set by `specfuse-xentity-shape`, so the exemption cannot be taken silently. Runtime enforcement of the write policy is generator-side and **not implemented** — see follow-up #12. (2) **Null-crash fixes** (authoring #14) — the upstream `duplicated-entry-in-enum` rule aborts the entire Spectral run on any null in the document, emitting zero findings, which reads as a clean pass; it is now disabled and replaced by the null-safe `specfuse-no-duplicate-enum-entries`, and 26 of the kit's own filter expressions gained the `@ &&` guard they were missing. (3) **`scripts/spectral-lint.sh`** (NEW) — every CI invocation now fails on a crash or an empty report, not only on reported findings. (4) Documentation: `$ref` resolution sensitivity and the resolved-vs-unresolved audit (#12), the per-rule baseline ratchet for adopting the ruleset against existing specs (#15), the value-object-invariant vs entity-business-rule boundary for `validationRules` (#13), and the requirement that extension-shape changes update `schemas/spectral/` in the same PR (#13). **Spec-author action:** none required — every change is additive or a fix. Projects that forked the kit rulesets should re-apply the null guards and the `duplicated-entry-in-enum` replacement, since a forked copy keeps the crash. |

| `v0.5.6` (incubating) | generator **`0.5.4`** (pin unchanged) | **Kit-only release — documentation and lint surface for a consumer-side contract change (authoring #19).** Adds the relationship-classification and projection vocabulary that became necessary when implicit `{Entity}Id` -> `belongsTo` inference was retired in `0.5.4`: **`x-references`** (non-owning association FK; `none` for an opaque uuid, which requires a justifying description), **`x-fk-for`** (owning FK under a name that is not `{Entity}Id`, bound to a declared `belongsTo`), and **`x-expand-of`** / **`x-projection`** (read-only scalar and collection projections, excluded from persistence). **Convention shift:** the former "`belongsTo` wins, `x-references` degrades to a hint" precedence is removed — `x-references` is association-only, and declaring it alongside a `belongsTo` for the same target is an error. Documented in `Vendor_Extensions.md` §1.7-1.9 and `API_Handbook.md` §9.5 (including the rule that a nested `/{parents}/{parentId}/{children}` route requires the child to `belongsTo <Parent>` — an association is not sufficient). Seven Spectral rules ship with it, covering marker shape, marker exclusivity, the opaque-reference justification, projection coherence, and the no-markers-on-derivatives rule; a fixture asserts each fires and that a correctly-marked schema trips none of them. Graph-level validators (`DDD_UNCLASSIFIED_ENTITY_RELATIONSHIP`, `DDD_DUAL_RELATIONSHIP_DECLARATION`, `DDD_INVALID_FK_FOR`, `DDD_UNMARKED_PROJECTION_EMBED`, `DDD_EXPAND_OF_NAV_MISMATCH`, `OPERATION_PARENT_NOT_OWNER`) need the resolved entity graph and remain generator-side — see follow-up #13. **Spec-author action:** classify every FK-shaped uuid property explicitly; mark read-only embeds. Existing specs do not change meaning — an unmarked property was already unclassified after `0.5.4`. |

| `v0.5.7` (incubating, current) | generator **`0.5.4`** (pin unchanged) | **Kit-only release — PATCH child-collection reconcile contract (authoring #20).** A collection property present in an `Update{Resource}` body is the **complete desired set**; there is no merge mode. Value-object and scalar arrays are replaced wholesale; child-entity collections reconcile by identity — known `id` updates in place with the PK preserved, no `id` creates (validated as a create, a violating element fails the whole PATCH with `400`), unknown `id` is `404`, and a tracked child absent from the array is **permanently hard-deleted** (reconciled children are aggregate-internal state, so a project-wide soft-delete convention for top-level resources does not extend to them — consumer-facing descriptions must say omitted children are not recoverable). Fenced by the already-mandatory `If-Match` and `?validateOnly=true`. Documented in `API_Handbook.md` §1.5.1, which also records the rejected alternatives (upsert-only, `_delete` markers) so the convention does not drift back. **DTO shape rule:** an `Update{Child}` used as an array element inside an `Update{Parent}` must expose an **optional** `id`; `specfuse-child-collection-reconcile-id` (WARN) enforces it, scoped to exactly that shape so it does not flag the many `Update*` DTOs whose identity lives in the URL path. Generator-side runtime behaviour and the fail-closed generation guard are follow-up #14. **Spec-author action:** add the optional `id` to child Update DTOs, and correct any property description documenting upsert-only or absent-means-untouched semantics — prose that disagrees with the server is worse than no prose. |
...
### 13. Relationship classification and projection markers (kit `v0.5.6`, authoring #19)
...
### 14. PATCH child-collection reconcile semantics (kit `v0.5.7`, authoring #20)

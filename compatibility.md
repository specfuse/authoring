...
| `v0.10.0` (incubating) | generator **`0.5.8`** — unchanged; `Specfuse/generator-dist` release `v0.5.8`, asset `specfuse-generator-0.5.8.jar`, sha256 `517795ce…97c97` | **Kit release — the kit stops claiming a directory it does not own, and a scaffolded project finally passes its own validator.** *Minor, not patch: kit tooling moves from `scripts/` to `scripts/specfuse/`, which is breaking for any project that invokes those paths from its own CI or docs.* **No pin change.** **(1) BREAKING — `scripts/` → `scripts/specfuse/`.** The kit shipped ~20 generically-named scripts (`validate-specs.sh`, `bundle-spec.sh`, `serve-docs.sh`) straight into the consumer's `scripts/`, and a project file colliding with one was overwritten on upgrade with only a stderr warning. Deletion was already manifest-scoped and safe; **writes were not**. Ownership is now structural: `OVERLAY` and `PRUNE_DIRS` are scoped to the subdirectory, so an upgrade cannot walk the project's `scripts/` at all — which also protects the shared substrate's `validate-event.py` / `validate-frontmatter.py`, which live there and the kit does not own. **`upgrade` migrates automatically**, deleting the old copies under the same rule prune uses (only what the manifest proves the kit wrote), so a project file that merely shares a name is never touched. **What the kit cannot fix for you:** your own references to `./scripts/<name>` in CI, Makefiles, docs or editor tasks. Re-point them at `./scripts/specfuse/<name>`; there is no shim. **(2) A scaffolded project failed its own `validate-specs.sh`.** Two anonymous `type: object` placeholders in `errors.yaml` (`Error.details`, `PreconditionFailedBody.currentResource`) were rejected by the generator's `ANONYMOUS_OBJECT_PLACEHOLDER`. Both are genuine open maps no `$ref` can name, now declared with `additionalProperties`; `API_Handbook.md` records them as the only sanctioned open maps and why they do not soften the event-payload ban. It hid because that check needs the private jar, so CI never ran it — a jar-free YAML shape guard now covers it. **(3) Ten `given` expressions normalised** from `$.paths[*].[verb,…]` to the bracket form, with a CI grep against reintroduction; a consumer reported the dotted form leaking outside `paths`, which did **not** reproduce here (follow-up 29). **(4) First consumer measurement of `info.x-services`** corrected our adoption guidance: quote `holds` pairs, not crossing edges — a five-service split cut pairs 99 → 36 but edges only 168 → 138. |
| `v0.11.0` (incubating) | generator **`0.6.0`** — `Specfuse/generator-dist` release `v0.6.0`, asset `specfuse-generator-0.6.0.jar`, sha256 `e821f045…b7e658` | **Pin bump + the `x-protection` axis lands.** *Minor, not patch: a classified property that validated under `0.5.8` now fails until it declares an at-rest decision.* **(1) `x-protection` (NEW property extension).** `0.6.0` splits sensitivity into two axes — `x-classification` for what the data is, `x-protection` for how it is handled — and gates the second at ERROR **with no grace period** (proposed for demotion and declined by the generator owner, 2026-08-21, on the grounds that a hard gate beats retrofitting protection decisions onto an authored spec). Sub-keys: `atRest` (`none｜encrypted｜hashed｜never_persist` — note the underscore), `mode` (`randomized`; `deterministic` parses and is then refused as unsupported in v1), `blindIndex`, `masking`, `unbounded`, `rationale`, `reviewedOn`, `reviewedBy`. The kit mirrors four of the generator's rules so a consumer is told in the editor rather than at generate time: `specfuse-xprotection-shape`, `-atrest-required`, `-none-requires-rationale`, `-mode-deterministic-unsupported`. **Spec-author action: every property carrying `x-classification` must add `x-protection.atRest`**, and `atRest: none` must carry a `rationale` — it is a decision, not a default. **(2) `x-entity.valueObjects[*]` gains `classification` and `protection`.** The guard is `additionalProperties: false`, so omitting them would have made the feature unadoptable; caught by the pin-bump vocabulary gate before release, which is what that gate exists for. **(3) The `exposed` mutex is CORRECTED, not merely widened.** §26's set was authored in `0.10.0` by reasoning against a jar that had no opinion, and `0.6.0` disagrees on two tokens — measured: `[cardholder, exposed]` is **accepted** by the generator (the kit was rejecting it — the forbidden direction) and `[confidential, exposed]` is **rejected** (the kit was accepting it). The set is now `sensitive`/`confidential` plus an `encrypted`/`hashed` `atRest`, mirroring `PROTECTION_EXPOSED_MUTEX`. `credential` and `sad` leave the set because the generator owns them through different rules with different remedies (`PROTECTION_CREDENTIAL_REQUIRES_WRITE_ONLY`, `PROTECTION_SAD_MAPPED`). **(4) `x-classification: [encrypted]` survives.** FE-G02 did not land; `encryptedProperties` is still the only key the AI-read exclusion reads (FE-G04 also not landed). All 15 fixture cases were cross-checked against the jar in both directions. |

| `v0.12.0` (incubating) | generator **`0.7.0`** — `Specfuse/generator-dist` release `v0.7.0`, asset `specfuse-generator-0.7.0.jar`, sha256 `5eb35c18…9d8365` | **⚠ BREAKING contract change — the encryption axis collapses to one declaration.** Generator 0.7.0 retires **both** of the declarations that used to record an at-rest decision, and this kit release is the one the ruleset spent a version advertising the migration for. **(1) `x-classification: [encrypted]` is rejected** (`INVALID_EXTENSION_VALUE`) at the property level *and* at the `x-value-object.classification` usage site; the closed set is now the eight values `pii`, `sensitive`, `confidential`, `exposed`, `financial`, `credential`, `cardholder`, `sad`. **(2) `x-entity.encryptedProperties` is retired outright** (`ENTITY_INVALID_CONFIG`) — no field, no accessor, no derived view. **Replacement for both: `x-protection: {atRest: encrypted}` on the property.** Kit-side, `specfuse-classification-encrypted-superseded` is **promoted from `warn` to `error`** and its message inverts from *"keep it for now"* to the replacement — the promotion condition its own comment named (*"the release that pins a generator where FE-G02 and FE-G18 ship together"*) is exactly what 0.7.0 satisfies. That pairing is what makes the migration safe rather than merely announced: 0.7.0 also taught `SENSITIVE_FIELD_IN_RESPONSE` to accept `atRest: encrypted` as a satisfier, so a `portalPassword`-shaped property does not begin failing as you move it. The rule is also now scoped to arrays via `if/then` — `not: {contains}` is FALSE for a non-array in JSON Schema, so it used to fire on a malformed scalar `x-classification: pii` and report a token the author never wrote; harmless at `warn`, a blocking false message at `error`. `encryptedProperties` is gone from `specfuse-xentity-shape`, with a `RetiredEncryptedProperties` fixture case asserting the rejection. **⊕ Also in 0.7.0, no kit action:** the `extensions` command's published `x-entity` vocabulary narrows accordingly; `applicationServiceGateTest` is retired as a registerable artifact type (fold it into `authMatrixTest`); a new `reachability` subcommand audits a `project.json` for unregistered guarantee-bearing artifacts; generated Python honours `x-entity.delete` (soft-delete reads, archive-on-delete, an owning-chain filter at depth 1); and TypeScript repository-fake and Azure-Functions envelope fixes (#1296, #1185). **Spec-author action, required before regenerating:** replace every `x-classification: [encrypted]` with `x-protection: {atRest: encrypted}` on the same property, keeping the remaining classification values, and delete every `x-entity.encryptedProperties` array, declaring `atRest: encrypted` on each property it named. Both now fail `validate`, so the migration is not optional. `aiAccess.readableProperties` exclusion is resolved from the protection axis, so the implicit read surface is unchanged for a correctly-migrated spec. |
| `v0.13.0` (incubating) | generator **`0.7.0`** — unchanged; `Specfuse/generator-dist` release `v0.7.0`, asset `specfuse-generator-0.7.0.jar`, sha256 `5eb35c18…9d8365` | **Ruleset migration tooling, and two corrections found by running the pinned jar instead of reading it.** *Minor, not patch: `spectral-overlay-diff.py` is new kit-owned tooling delivered into `scripts/specfuse/`.* **No pin change.** **(1) The complete `rm-*` → `specfuse-*` rename map ships as data** at `schemas/spectral/rule-renames.yaml` (follow-up 1). §1 tracked twelve renames; the other ~160 were mechanical and recorded nowhere, so migrating meant guessing the prefix swap — and **two rules are exceptions**. Two tools read the map: `spectral-ratchet.py --migrate-rule-ids` rekeys a committed baseline rather than re-baselining it away, and refuses to empty it, to merge two counts onto one id, or to run against a stale map; `spectral-overlay-diff.py` classifies a project ruleset as redundant / diverged / project-specific / kit-only-new. **(2) `spectral-overlay-diff.py` is DELIVERED into projects** at `scripts/specfuse/`, unlike the ratchet — it is the one tool you run *during* an upgrade, when fetching a file from another repository is the least useful instruction the kit could give. `schemas/README.md` gains §"Reducing an overlay that forked from the kit", the case that actually occurs for any project bootstrapped by copying another's ruleset: adoption means deleting duplicates, and extending the kit without doing so double-reports every shared rule. **(3) `x-protection.atRest` accepts both separators** (follow-up 33). The generator normalises `never_persist` and `never-persist` to one value and `extensions` prints the hyphen, so the kit was rejecting the spelling its own published vocabulary advertises. Widening the enum alone would have silently disarmed `specfuse-xprotection-never-persist-not-mapped`, whose `notMatch` was anchored on the underscore — both halves land together, pinned in both directions by fixture. **(4) `patch-reconciled` is corrected to UNUSABLE** (follow-up 32). §30's follow-up claimed `clabonte/generator#1219` was fixed, reasoning from an error-message wording change; measured against the pinned `0.7.0` jar, the check still rejects the canonical `Update{Child}` shape the contract mandates. The guard enum is unchanged — the token is legal — but the handbook now tells authors to declare `reason: other` with a `reasonText` naming the blocker. **Spec-author action: none required.** (3) removes a false positive and (4) is documentation; a spec that validates today still validates. |
| `v0.13.1` (incubating) | generator **`0.7.0`** — unchanged; `Specfuse/generator-dist` release `v0.7.0`, asset `specfuse-generator-0.7.0.jar`, sha256 `5eb35c18…9d8365` | **Patch — two corrections to one handbook passage, both measured against the pinned jar.** Documentation only; no rule, fixture or schema change. **No pin change.** **(1) `SENSITIVE_FIELD_IN_RESPONSE` only sees `x-entity` schemas, and §1.6 now says so.** It read as though response DTOs were covered — *"such a field on a response DTO is a likely credential leak"* — which is the opposite of what the rule does. Measured in one document, one run: `Account.accountSecret` on an entity schema is flagged; `TokenResponse.deviceSecret` on an ordinary response schema is not. A response DTO that mints a bearer token has no entity behind it to be checked, **and nothing in the kit or the generator guards it** — the passage now names that limit and tells the reader not to read a clean validation run as coverage. Reported by a consumer against three such flows (`clabonte/generator#1233`). **(2) `x-classification: [encrypted]` is no longer a satisfier and has been removed as one.** `0.7.0` retired the token — the retirement kit `0.12.0` shipped the migration for — so writing it yields `INVALID_EXTENSION_VALUE` + `PROTECTION_ATREST_REQUIRED`. It stops the rule firing only because the document stops validating, which is not what *passes the rule* means to a reader. Replaced with `x-protection: {atRest: encrypted}`, verified clean and satisfying. The same section already described the `0.7.0` satisfier correctly ten lines earlier, so its two halves disagreed. **Spec-author action: none** — but if you return credentials from response DTOs, review them by hand; no rule sees them. |
| `v0.14.0` (incubating) | generator **`0.8.0`** — `Specfuse/generator-dist` release `v0.8.0`, asset `specfuse-generator-0.8.0.jar`, sha256 `6d3e6b02…89544` | **Pin bump — three vocabulary additions, and the guard that was supposed to catch them caught one.** *Minor, not patch: it adds to the contract vocabulary (`x-entity.uniqueBy`, `concurrency: delegated`, `delete.archiveVisibility`) rather than only correcting what `0.13` shipped.* **(1) `x-entity.uniqueBy` (NEW key).** A list of constraint objects, each `{ name?, properties[] }`, so an entity can declare several uniqueness constraints and each can be composite — the limitation that retired the deprecated `x-unique` (`FEAT-2026-0148`; the kit never documented that key, so there is nothing to migrate). `properties` is required and non-empty, members must name properties of the entity spelled exactly, and dotted paths do **not** resolve — the opposite of `filterableProperties`. New rules: `UNIQUE_BY_UNKNOWN_PROPERTY`, `UNIQUE_BY_DUPLICATE_INDEX_NAME` (both ERROR), `UNIQUE_BY_CENSUS`. **(2) `x-entity.concurrency: delegated` (NEW value + `to`).** A third mode for a child written only under its parent's `If-Match`: `{ mode: delegated, to: <Ancestor> }`. It has no bare-string form, `to` is required by it and legal on nothing else, and `reason`/`reasonText` are refused beside it — the delegation is the justification. `DELEGATION_TO_UNRESOLVABLE` (ERROR) and `TO_NOT_IN_BELONGS_TO` (WARNING) are generator-side; the guard checks the shape. Entities that declared `none` with a `reason` because there was no truer option can now say the true thing. **(3) `x-entity.delete.archiveVisibility` (NEW member).** `operational` | `restricted`, on `soft` only (`ARCHIVE_VISIBILITY_ON_HARD_DELETE`, ERROR). Absent resolves to the fail-closed `restricted`, so no existing entity changes meaning; `ARCHIVE_VISIBILITY_UNDECLARED` (WARNING) nudges every soft entity that has not decided. Validated, not yet enforced — like the rest of delete gate 1, no read filtering is generated. **(4) `patch-reconciled` is usable again — `clabonte/generator#1219` landed.** `0.13.0` §(4) corrected this member to UNUSABLE after measuring `0.7.0`; re-probed against `0.8.0` on the same probe, the canonical `Update{Child}` collection shape now satisfies `DELETE_REASON_CONTRADICTED`, as do the entity itself and the `Basic{Child}` DTO, while an entity with no parent collection still draws the ERROR. The handbook now documents the migration off the `reason: other` workaround. Also corrected: `DELETE_REASON_MISSING` was recorded as *reserved; not raised* — it fires as a WARNING on both `0.7.0` and `0.8.0`, so the row was stale rather than the behaviour new. **(5) The vocabulary gate compared keys only, and missed two of the three additions.** `check-extension-vocabulary.py` read `extensions --format json` for top-level `key` and discarded `members` and `values`, so only `uniqueBy` was reported; `delete.archiveVisibility` (a member) and `concurrency: delegated` (a value in a closed enum) walked straight past the guard whose entire purpose is to stop them. Both directions are now compared and both are pinned by CI assertions. A guard that reports one finding out of three reads exactly like a guard that works. **Spec-author action: none required** — every addition is additive with a backward-compatible default. |
| `v0.15.0` (incubating) | generator **`0.9.0`** — `Specfuse/generator-dist` release `v0.9.0`, asset `specfuse-generator-0.9.0.jar`, sha256 `68bfff93…cbc130` | **Pin bump — the protection declarations get a deliverable.** *Minor, not patch: it documents a new registerable artifact type, which is an addition to the project-file surface.* **No vendor-extension vocabulary change.** `extensions --format json` is byte-identical between `0.8.0` and `0.9.0` apart from set ordering, and the vocabulary gate passes against `0.9.0` with no widening — the first release in a while where the guard had nothing to say, which is what a clean pin bump looks like. **(1) `dataProtectionAudit` (NEW markdown artifact).** Emits `data-protection-audit.md` and a JSON companion carrying the same record, rendered from the `x-classification` / `x-protection` declarations already in the specifications: data inventory, protection matrix, exceptions register, key management, egress map, erasure capability. The egress section is the one that reads across specs — snapshot, event payload, `aiAccess` readability and API response, per classified field. Three findings are reported and **none fails the build**: reviews older than 18 months, classified fields reaching a snapshot with no `x-snapshot-pii-acknowledged` entry, and fields disclosed to an agent by an explicit `aiAccess.readableProperties` listing the implicit expansion would have withheld. **It is not emitted unless registered** in a group's `artifacts[]`, and nothing warns you at generation time — the generator's `reachability` subcommand is what audits for that. Documented in `Project_File.md` §11.5 with the registration snippet, and pointed at from `Vendor_Extensions.md` §1.5 where the declarations are authored, since the deliverable is the reason to fill `rationale` / `reviewedBy` / `reviewedOn` in properly. `reviewedOn` older than 18 months is now load-bearing; that row says so. **(2) `x-snapshot-pii-acknowledged`: the map form now has a visible consequence.** The audit's *"why each acknowledged disclosure was accepted"* list is built from the map form only, so a field acknowledged in list form renders as acknowledged with no reason beside it. The kit's `specfuse-async-snapshot-guardrails` already flagged the bare list at WARNING and the `BareListAck` fixture already pinned it — no rule change; `AsyncAPI_Handbook.md` §4.2 now says what the list form costs you downstream. **Deliberately NOT claimed:** the jar's diagnostic says the list form *"is still accepted but states no reason"*, and this release does not record that as behaviour — §32's lesson. What was measured is that on `examples/hello-orders`, `validate` reports nothing about snapshot PII whether the acknowledgement is a map, a bare list, or absent entirely, so a clean `validate` is not evidence of acknowledged disclosures. The kit's Spectral rule covers it. **⊕ Unrelated, pre-existing, and not fixed here:** `examples/hello-orders` does not `generate` — `OperationResolutionException` on `PlaceOrder` (*"is a Create but has no form parameters or body parameter of type Entity/ApiModel"*). Identical under `0.8.0` and `0.9.0`, so it is not a regression; CI only validates and lints the example, which is why nothing caught it. **Spec-author action: none required.** Projects wanting the compliance document register the artifact; everything else is unchanged. |
| `v0.16.0` (incubating) | generator **`0.9.0`** — unchanged; `Specfuse/generator-dist` release `v0.9.0`, asset `specfuse-generator-0.9.0.jar`, sha256 `68bfff93…cbc130` | **Kit release — three rules the pinned generator disagreed with, and a scaffold that disabled a check.** *Minor, not patch: it REMOVES two Spectral rule ids, which is breaking for any consumer baseline that names them, and it changes the shape of the `project-init` template.* **No pin change** — the generator is still `0.9.0`; everything here is the kit catching up to a jar it had already pinned. **(1) `428` is unauthorable (§34).** The kit required a `428` on every write while also requiring `If-Match: required: true` — mutually exclusive at runtime, since an absent required header is a `400` from model validation before any gate runs. The generator has forbidden the declaration since `0.8.0` at ERROR (`PRECONDITION_REQUIRED_DECLARED_UNEMITTED`), so the kit was requiring what the pinned jar fails the build on. `specfuse-428-on-write` inverted to **`specfuse-no-428`**, paired with **`specfuse-no-428-in-prose`** because a declaration rule does not check the sentence that motivated the declaration — a consumer clears `responses.428`, goes green, and keeps shipping descriptions promising a status the API cannot emit. Both `warn`; the jar already errors, so Spectral severity only picks who tells you first. **(2) `specfuse-etag-on-get` retired (§35).** It keyed on path shape and demanded an `ETag` that `ENTITY_TAG_DECLARED_UNGATED` (ERROR since `0.8.0`) rejects whenever the response entity is not `optimistic`. Declaring the header and dropping it were both findings, from different tools; no authorable state satisfied both. A migration, not a deletion — `ENTITY_TAG_UNDECLARED_GATED` covers the same direction at ERROR, keyed on the entity rather than a path regex. **(3) Ten concurrency/ETag diagnostics documented (§35)**, including an unnamed *"(ETag obtainability)"* placeholder row that is `CONCURRENCY_PRECONDITION_UNENFORCEABLE`, the four `CONCURRENCY_PRECONDITION_*`, the five `CONCURRENCY_DELEGATION_INVALID` sub-reasons, and the four `ENTITY_TAG_*`. **(4) The two other soft-delete overlap rules (§36)** — `DELETE_SOFT_BOOLEAN_FLAG_OVERLAP` (`0.7.0`, undocumented for two kit releases) and `DELETE_SOFT_TIMESTAMP_OVERLAP` (`0.8.0`), the latter with the caveat it needs: it matches three literal property names and asks nothing about whether the field is read elsewhere, so its "remove or rename" fix text is destructive on a domain fact. **(5) `info.x-roles` is the role registry (§37), and the scaffold was disabling it.** `Vendor_Extensions.md` §3.1 sent authors to a `common/enums.yaml` enum and claimed Spectral enforced membership; neither was true. The example and the `project-init` template both followed it — earning `OPERATION_ROLE_REGISTRY_MISSING` (the check disables itself, so a typo'd role passes `validate` silently) plus `SCHEMA_UNREFERENCED` for the dead enum. Both fixed; the dead `Role` enum is gone from both, and `examples/hello-orders/api/specs/v1/common/enums.yaml` with it. **Migration:** `rm-428-on-write` and `rm-etag-on-get` are now `retired:` in `rule-renames.yaml`, not `renames:` — an inverted rule's baseline count is the complement of its successor's, so carrying it seeds a ceiling that means nothing. `retired:` entries gained a `note:`, printed by `spectral-ratchet.py --migrate-rule-ids`, and the overlay classifier grew a `RETIRED BY THE KIT` bucket because reporting such a rule as project-specific reads as "keep it". **Spec-author action:** delete every `428` response and every `428` promise in an operation `description`; drop the `ETag` header from any safe read whose entity is not `optimistic`; declare `info.x-roles`. The first two are what the pinned jar already fails on; the third turns on a check you do not currently have. |
| `v0.17.0` (incubating) | generator **`0.11.0`** — `Specfuse/generator-dist` release `v0.11.0`, asset `specfuse-generator-0.11.0.jar`, sha256 `7540c8e1…6a9fc0` | **Pin bump — no new authorable key, and the vocabulary gate misread a path it had always accepted.** *Minor, not patch: it documents fifteen deprecated artifact ids and their replacements, which is a change to the project-file surface.* Generator `0.10.0` was never published to `generator-dist`; `0.11.0` carries everything it did. **(1) No vendor-extension vocabulary change an author can write.** Diffing `extensions --format json` between `0.9.0` and `0.11.0`, flattened to member paths: `valueObjects.protection` is now printed as `valueObjects.protection.atRest` with the same four values, `concurrency.to` appears, and the `info` surface (`x-domains`, `x-roles`, `x-services`) is newly published. All three are generator `#1533` printing nested members by their real path, not new keys. **(2) The vocabulary gate reported `protection.atRest` as rejected, and it was not.** The ruleset has always accepted `atRest` under `valueObjects[*].protection`; the check compared the whole dotted string against property names, and no property is spelled with a dot. The value direction had the same defect and was worse: `_descend` found no property named `protection.atRest` and skipped the member as absent, silently, so `atRest`'s closed set was never compared. `check-extension-vocabulary.py` now splits a dotted member into one step per property in both directions, with a regression case in CI beside the `delete.archiveVisibility` one. The comparison it newly performs reports one informational warning, not a failure: the ruleset also accepts `never_persist`, which the published set spells only `never-persist` — deliberate, per the comment on the rule, because the generator normalises separators. **(3) `Project_File.md` §11.3/§11.4 re-snapshotted against `0.11.0`, plus §11.4.1.** Fifteen ids are now temporary aliases that still generate the same files and log a `WARN` naming the replacement: `dartDto` → `dto`; `dartEntityBuilder` / `dartNewDtoBuilder` / `dartUpdateDtoBuilder` → `entityBuilder` / `newDtoBuilder` / `updateDtoBuilder`; `dart`/`typescriptSerializationTest` → `serializationTest`; `dart`/`typescriptValidatorTest` → `validatorTest`; `dartApiClientTest` → `dartEtagTest`; `dartMutationProvider` / `typescriptMutationComposable` → `mutationBinding`; the two mutation tests → `mutationBindingTest`; `dartJsonFixture` / `typescriptFixture` → `fixture`. The §11 example's `artifacts[]` is migrated. **(4) Measured on `examples/hello-orders`:** `validate` on the bundled OpenAPI reports the same 28 findings under `0.9.0` and `0.11.0`. The bundled AsyncAPI reports `PARSE_ERROR` under both jars, so it predates this pin. **Not kit contract, but in the jar this pins:** generated C# types `format: date` as `DateOnly` and serializes it `yyyy-MM-dd`, so every entity date column needs a `datetime2` → `date` migration; an identifier with a digit before a capital (`pbkdf2HmacSha256`) now keeps that word boundary in generated names; `x-sample` `provider: template` renders a value instead of carrying its `{{…}}` text. Details in the generator's `CHANGELOG.md` under `0.11.0`. **Spec-author action:** none on specs. **Project-file action:** migrate the deprecated ids in `artifacts[]` before the aliases are removed. |
| `v0.18.0` (incubating) | generator **`0.12.0`** — `Specfuse/generator-dist` release `v0.12.0`, asset `specfuse-generator-0.12.0.jar`, sha256 `74d42170…9f35091` | **Pin bump — conditional GET becomes a contract, plus four consumer handoffs.** *Minor, not patch: it adds two Spectral rule ids and makes `hasPrev`/`hasNext` required on the list envelope, a schema-shape change.* **(1) No vendor-extension vocabulary change.** `extensions --format json` is identical to `0.11.0` apart from set ordering, and the vocabulary gate's report is byte-identical. The new `x-` strings in the jar (`x-entity-request-body`, `x-ts-conditional-read-*`, `x-dart-date-only*`) are set by the generator for its own templates. None is read from a spec. **(2) Conditional GET is enforced, at ERROR.** New `ConditionalReadDeclarationValidationRule` has five ERRORs and a census: `If-None-Match` and `304` must be declared together, only on `GET`/`HEAD`, with no body on the `304` and an `ETag` on a 2xx. The pair checks run on **every** operation, `x-manual` included, so **a spec that declares either half alone fails `validate` on this pin**. `API_Handbook.md` gains a Conditional GET section, taken from `restomanager-specs`' handoff. Kit Spectral mirrors all five at `warn` (`specfuse-conditional-read-pair`, `-shape`), plus a kit-only check against a `412` beside the `304`. Fixture `conditional-read.yaml` is asserted in both directions in CI. Kit and jar agree on all six violations and all four accepted shapes, including a lower-cased `$ref`'d header and a path-level parameter. Generated TypeScript and Dart clients gain a `ConditionalResult<T>` conditional-read method. **(3) Two new concurrency WARNINGs**, now in `Vendor_Extensions.md` §1.1: `CONCURRENCY_PRECONDITION_UNROUTED` fires when an `If-Match`/`412` write reaches its `optimistic` target only via `x-operation.target`, so no gate is generated. Through `0.11.0` the census counted these as gated. `ENTITY_TAG_UNOBTAINABLE_PROJECTION` fires when a safe read returns a projection of an `optimistic` target, so no `ETag` can be published. **(4) New C# artifact id `fieldEncryptionConformanceTest`.** `Project_File.md` §11.1 is re-snapshotted; it was last taken in May and also lacked the four `heldEntity*` ids from `0.6.0`. **(5) Handoffs landed with this release:** `hasPrev`/`hasNext` required (#105); §1.4.1, PUT erases fields the client cannot read back, and the proposed "absent = keep" is not adopted because the generated replace does the opposite (#106, follow-up 39); four stale pin claims retired, service-topology severities re-verified, and `x-derived-from` recorded as a generator ask (#107, follow-up 40). **(6) Measured on `examples/hello-orders`:** `validate` on the bundled OpenAPI reports 28 findings under `0.11.0` and 29 under `0.12.0`. The one addition is `CONDITIONAL_READ_CENSUS` (SUGGESTION, 0 conditional reads). **Spec-author action:** search for a lone `If-None-Match` or `304` before upgrading. On a hand-written backend, emit `hasPrev`/`hasNext` on every list response. **Project-file action:** none. |

| `v0.19.0` (incubating) | generator **`0.12.0`** — unchanged; `Specfuse/generator-dist` release `v0.12.0`, asset `specfuse-generator-0.12.0.jar`, sha256 `74d42170…9f35091` | **The field-encryption authoring surface, written up — five consumer reports, all measured against the pinned jar.** *Minor, not patch: it widens a Spectral shape and publishes vocabulary the handbooks had never carried.* **No pin change.** **(1) `Project_File.md` §15 — the `encryption` block.** `profiles` and `keyScope` have been parsed since the `x-protection` axis landed in `0.6.0` (`EncryptionDefinition`, `Project.populateEncryptionProfiles` / `populateEncryptionKeyScope`) and were documented nowhere. The section's load-bearing sentence is that **absent means no floors, silently** — a project can classify a field `credential`, declare `atRest: none` with a one-word rationale and validate clean, and nothing distinguishes that from a project whose floors are satisfied. Three profiles, composed as a **union of floors, not a ladder**: `law25` (`PROTECTION_LAW25_FLOOR`), `pci` (`PROTECTION_PCI_FLOOR` — `cardholder` needs `masking.first <= 6` / `masking.last <= 4`, `sad` needs `never-persist`) and `soc2` (`PROTECTION_SOC2_METADATA_REQUIRED` — `rationale`, `reviewedOn` and `reviewedBy` all mandatory). `keyScope` is `tenant` \| `global`; **absent disables both** the FE-G26 tenant-scope check and the generated `DeclaredKeyScopeMismatchException`, verified in bytecode (`ifnull` before the `equalsIgnoreCase("tenant")`). Numbered 15, not inserted after §6, so no existing `§7`–`§14` cross-reference moves. Reported as authoring #110. **(2) `Vendor_Extensions.md` §2.1 publishes `classification` and `protection` on a `valueObjects` usage site,** which `ValueObjectConfiguration` has carried since the `0.6.0` pin while §2.1's schema listed neither — so an author reading the published schema concluded they did not exist. With the per-usage-site argument (a shared address definition is a natural person's home at one site and a business location at the others, so the site is the only place the true statement can be written), the scalar/per-member disambiguation (`ProtectionDefinition.isSubKey` over the key set) and its one sharp edge (a member *named* like a sub-key forces the scalar reading). **The queryable consequence is a build ERROR, not a silent trade-off**: `VALUE_OBJECT_ENCRYPTED_QUERYABLE` (FE-G28) fires on a non-empty **effective** queryable set — the usage site's if declared, the definition's `defaultQueryable` otherwise, which is why the fix is `queryable: []` rather than removing the key. Reported as authoring #111. **(3) The ruleset accepts the per-member shape of `protection`, which it previously rejected.** `x-entity.valueObjects[*].protection` was schema'd as the scalar form alone under `additionalProperties: false`, so the per-member map the jar accepts failed kit lint — the adoption-blocking direction. Now a `oneOf` over the scalar anchor and a map of it; `specfuse-xprotection-shape` still aliases the scalar branch, unchanged. Four fixture cases in `xentity-shape-keys.yaml`, asserted in both directions in CI: `ValueObjectProtectionScalar` and `ValueObjectProtectionPerMember` clean, `ValueObjectProtectionUnknownSubKey` and `ValueObjectProtectionMemberNotObject` flagged. **(4) `x-protection.unbounded` said what its own name says.** The row now states what declaring it asserts (no `maxLength`, deliberately, so no bounded ciphertext column can be sized and storage falls back to the large-object form), that at this pin its only consumer is the audit's protection matrix, and the rule an author cannot otherwise reach: **a `pattern` is not a bound** — `pattern: '^[0-9]{5}$'` pins the length exactly and still leaves the property unbounded for sizing. Reported as authoring #112; the ciphertext-column width formula in that report belongs to a later generator and is **not** published here (see below). **(5) §1.5 gains *“Renaming an encrypted property is a data migration, not a rename”*.** Verified against the pinned `EncryptionContext`: `AAD = envelopeVersion + tenantId + entityName + propertyName + entityId`, derived from the current model on both paths, so a rename makes every stored value for that column unreadable in one deploy — as does renaming the entity or rewriting a row's primary key. The four-step add / backfill out of band / verify-and-switch / drop-later shape, with the gap between steps 3 and 4 named as the rollback window. Reported as authoring #113; the value-object half of that report is held for the pin bump. **(6) §1.5 gains *“What `atRest: encrypted` emits, measured against generator `0.12.0`”*, and a stale claim is retired.** §1.5 promised that declaring it was how *“encryption converters”* got emitted. It is not: the C# encryption library is emitted **unconditionally by `infrastructureProject`**, and no entity, EF-configuration, repository or converter template reads `x-protection` at all — `InfrastructureProjectArtifact` is the only class in the jar referencing those templates. At this pin the declaration drives validation, the audit and the `aiAccess` read-surface exclusion, and the persistence half is the consumer's to write, including a registration obligation nothing generated performs (`AddFieldEncryption()` is never called for you; `IDataKeyProvider` and `IKeyWrapper` are throwing placeholders that `ValidateOnBuild` deliberately does not catch). **Spec-author action:** declare `encryption.profiles` deliberately — if you have never declared it, you have no floors. **Not adopted here, held for the pin bump past `0.12.0`:** the ciphertext column-sizing formula and the value-object AAD table, both measured against generator `main` rather than this pin (authoring #114), and the runtime-contract handbook page (authoring #109). |

| `v0.20.0` (incubating, current) | generator **`0.13.0`** — `Specfuse/generator-dist` release `v0.13.0`, asset `specfuse-generator-0.13.0.jar`, sha256 `1a310efa…4607c8f` | **⚠ BREAKING for any spec already using `atRest: encrypted`.** *Minor: one new Spectral rule id, and a required schema member the previous pin did not ask for.* **(1) Field encryption becomes spec-driven.** Through `0.12.0` the C# encryption library was emitted **unconditionally by `infrastructureProject`** and **no** entity, EF-configuration, repository or converter template read `x-protection` at all — the declaration drove validation, the audit and the `aiAccess` read surface, and nothing else. `0.13.0` wires it up: six templates now read the axis, including `EntityTypeConfiguration`, `partials/property`, `partials/valueObject/flatten_configuration`, and two new interceptors (`FieldEncryptionSaveChangesInterceptor`, `FieldDecryptionMaterializationInterceptor`). Three runtime contracts ship that did not exist before — `IDecryptionFailureClassifier`, `IPoisonedFieldTracker`, `DecryptionOutcome`. So this is not "value objects join in": it is the release where `atRest: encrypted` produces storage at all, and a project that adopted the declaration on an earlier pin gets ciphertext columns it has never had. **(2) `unavailableProperties` is REQUIRED, at ERROR — the breaking half.** An entity declaring `x-protection.atRest: encrypted` on one of its own properties must also expose a `readOnly` string array named `unavailableProperties`, or generation aborts (`UNAVAILABLE_PROPERTIES_NOT_DECLARED`). The jar's reasoning: *"A poisoned read must have somewhere to surface which properties it could not decrypt."* Without it the only outcomes are an exception or an empty value materialised in place of a failed decrypt — and an empty value is written back by the save path as fresh, valid ciphertext, making the original unrecoverable. **Measured scope:** required for directly-declared encrypted properties only. An entity whose *only* encrypted target is a flattened value object does **not** need it, verified against a generated fixture — `0.13.0` emits per-member ciphertext there and asks for nothing. The kit mirrors the jar exactly and does not widen, because a lint rule stricter than the generator fails specs that generate cleanly. **(3) Kit-side:** new rule `specfuse-xprotection-unavailable-properties-required` (error), mirroring the jar through `openapiClassification.js`'s existing `check` dispatch; `Vendor_Extensions.md` §1.5 gains the section and validation-table row 15. Four fixture cases in `classification.yaml`, asserted in CI: `PoisonedReadMissing` fires; `PoisonedReadDeclared` and `ValueObjectOnlyEncryption` stay silent; and the long-standing `Customer` positive control **carries the migration itself** — it has two encrypted properties, so it stops being ERROR-free until the member is added, which is exactly what every adopting project will see. **(4) Two generator issues raised against the previous pin are fixed, both re-verified here rather than taken from the changelog.** `clabonte/generator#1993` — an encrypted value-object member with no declared `maxLength` used to size silently from an internal default; both the scalar and the value-object paths now warn with the same text, naming `unbounded: true` and the maximum bound. `clabonte/generator#1994` — the audit used to report an empty classification for a value object protected by a scalar usage-site declaration; **verified by generating an audit against a purpose-built fixture**, it now reports `pii` for the value object and for the scalar control alike, so `Project_File.md` §11.5's description of section 1 stands as written. **Spec-author action, required before regenerating:** search for `atRest: encrypted` and add `unavailableProperties` (`type: array`, `readOnly: true`, `items: {type: string}`) to every entity that declares it. Expect ciphertext columns and a schema migration where the previous pin produced none. **Not in this release, and tracked for the follow-up:** the ciphertext column-sizing formula (`maxLength × 4 + 46`, ceiling `1988`), the value-object AAD table, the all-or-nothing member cliff, usage-site-wins precedence, the closed canonical set for non-string targets, and the runtime-contract page that `0.13.0` finally makes writable — authoring #109, #112, #113, #114. |
...
| `{Resource}List` left `hasPrev` / `hasNext` optional | `restomanager-specs` handoff, 2026-09-04 | `required` gains both, in `API_Handbook.md` §1.7, `samples/endpoint-samples.yaml` and both `hello-orders` list schemas | Declared, described, and relied on — but not required, so a conformant server could omit them and no client could page on them. The generator never had the gap: its C# `PaginatedList` always serializes both (computed from `page` / `pageCount`), and its Dart `PaginatedNotifier` already documents `hasNext` as required. So the kit contract now matches what generated servers emit. The multiplier is the reason to read the note: every `{Resource}List` composes this envelope — the reporter counted 50 schemas behind 66 operations — so a hand-written backend that omitted the flags becomes non-conformant for all of them at once. Spec validation does not notice; response validation does. `links` stays optional. |
...
**Closed by generator `0.8.0` (kit `0.14.0`).** `#1219` landed. Re-probed 2026-08-31 against `specfuse-generator-0.8.0.jar`, sha256 `6d3e6b02…`, byte-identical to `generator.lock`, on the same one-variable probe: the `Update{Child}` DTO, the entity itself, and the `Basic{Child}` read DTO all satisfy the check, and an entity whose parent carries no collection of it still draws `DELETE_REASON_CONTRADICTED` — so the fix resolves the indirection without disarming the rule. The handbook's warning is replaced by a migration note off the `reason: other` workaround; nothing is forced, since `other` with a `reasonText` stays valid.
...
1. `extensions` prints `protection` as if it were a scalar enum by flattening its child `atRest`'s values onto it, and omits its other seven members. The JSON format already expresses nesting — `concurrency` uses `members` correctly — so this is a correct-use fix, not a format change. Until then *"`values` on a key"* cannot be read as *"this key takes one of these"*, which is the assumption the whole derive-from-`extensions` instruction rests on.
   **Kit accommodation, `0.14.0`:** `check-extension-vocabulary.py` now compares published VALUE sets, which walked straight into this: `protection`'s flattened `atRest` values read as a stale enum on the `protection` key and reported four false failures. The check now stays silent when an immediate child property carries the whole published set, which is the honest answer while the jar flattens — it cannot verify the mapping, so it does not claim to. When ask 1 lands the accommodation stops being needed; it is not load-bearing for anything else.

...
| `v0.7.0` (incubating, current) | generator **`0.5.6`** — `Specfuse/generator-dist` release `v0.5.6`, asset `specfuse-generator-0.5.6.jar`, sha256 `19bd5268…5dbec` | **Kit release — `x-entity.concurrency` enters the kit's vocabulary** (authoring #47, generator FEAT-2026-0078 / FEAT-2026-0088). *Minor, not patch: it adds to the contract vocabulary.* **(1) The key was missing from `specfuse-xentity-shape` entirely**, and that guard is `additionalProperties: false` — so **every** authored form failed lint at error severity, not only the scalar shorthand a consumer report described. The hand-patch that unblocked FEAT-2026-0078 rollout (recorded in follow-up §17) was applied in a consumer's ruleset, never upstream, so the kit still rejected the key it had documented the drift lesson about. **(2) The key is now accepted and shape-checked, mirroring the pinned generator exactly:** the bare scalar `optimistic`, and nothing else. Omitting the key is the documented opt-out, so `concurrency` is **optional** today. **(3) `none` and the object form are rejected, because the generator rejects them.** `EntityConcurrencyValidationRule` in the 0.5.6 jar matches `equalsIgnoreCase("optimistic")` on the stringified value and errors otherwise: *"The only accepted value is `optimistic` (`none` is reserved). Omit the key entirely to leave concurrency control off."* An earlier draft of this release accepted `none` and `{ mode, reason }` on the reasoning that a guard must never reject what the generator accepts — correct in principle, but applied to a generator version that does not accept them, which would have shipped a documented contract failing `validate` at ERROR on every declaration. The richer forms are FEAT-2026-0088, **unshipped as of 0.5.6**; the closed `reason` vocabulary is FEAT-2026-0091, also unshipped (no `reason` value literal appears anywhere in the jar). Both are documented in `Vendor_Extensions.md` §1.1 as a marked forward-looking block that authors are told not to write against yet. **Note the guard gap this exposed:** `check-extension-vocabulary.py` fails only when the generator knows a key the ruleset *rejects*, so a ruleset accepting a *value* the generator errors on is invisible to it. That direction is now pinned by fixtures instead (`BareConcurrencyNone`, `ConcurrencyObjectForm` must both be flagged). **(4) Handbook framing corrected.** `API_Handbook.md` §"Concurrency Control" scoped ETags to *"safe autonomous operations by AI agents"*; an approval workflow where an employee cancels a request while a manager approves it is two writers with no AI anywhere. Treating the AI-reachable set as the answer under-protects everything else, so the handbook now frames it as a floor. `Vendor_Extensions.md` §1.1 documents the key, the recommended `reason` vocabulary including `not-assessed`, and why `mutability` cannot supply it. **Spec-author action:** add `concurrency: optimistic` to every entity with more than one writer, and pair each with a single-resource `GET` — the generator flags an opted-in entity whose validator no safe operation returns. Start from the AI-writable set, then add approval workflows, shared rosters and anything else reachable by two callers; the AI-writable set is a floor, not the answer. **Do not write `concurrency: none`** — it is reserved and errors today; omit the key instead and record the decision in the entity description. See follow-up §18. | **(6) Generator re-pin 0.5.5 -> 0.5.6, and it is NOT a silent bump.** **⚠ Two validation rules ship at ERROR that did not exist in 0.5.5**, so a spec that validated clean against 0.5.5 can fail against 0.5.6. `PROJECTION_STRICTER_THAN_SOURCE` (generator #973/#976) — a `Basic{Resource}` read projection requiring a property the source schema leaves optional; unsatisfiable, because a record of the source may omit it and the response then violates its own schema. `VALUE_OBJECT_AUDIT_NAMED_PROPERTY` (#969/#976) — a value object declaring a date-time `createdAt`/`updatedAt`, which the generator silently drops from every generated type in all four languages. Both were promoted from WARNING at the restomanager-specs owner's request; measured on that repo the flip moved 7 findings from warning to error. **Spec-author action:** run `validate` before adopting — fix any `Basic*` that tightens `required` relative to its source, and rename an audit-named value-object property. **⊕ Also in 0.5.6:** `x-entity.concurrency` is now enforced at ERROR (`ENTITY_CONCURRENCY_INVALID`) for a misspelled sub-key or an out-of-set value. **This is narrower than (1)–(3) above describe.** The accepted set is the bare scalar `{ optimistic }` — `none` is reserved and the object form is compared as a string, so both error; omitting the key is the documented opt-out, and an absent `concurrency` remains legal. This is **not** FEAT-2026-0088, which would make the key *required* and add the `none`/`reason` forms, and is still unshipped. A second check ships with it, at WARNING: an entity declaring `concurrency: optimistic` that exposes an unsafe write but is returned by no safe operation cannot publish a validator, so every `If-Match` a client sends would be invented. A sub-resource DELETE no longer resolves to its path parent (#978, closes #974), removing false-positive `DELETE_SEMANTICS_*` findings on action-style and composite-key deletes. Plus the TypeScript surface: generated output now type-checks clean on a real bundle (#955), carries no unused imports (#963) or escaped quotes in JSDoc (#965), and repository fakes serve paginated list envelopes instead of throwing (#968).
...
**Status:** kit accepts and shape-checks the key against the **pinned** generator's contract (`specfuse-xentity-shape`, `Vendor_Extensions.md` §1.1, `API_Handbook.md` §"Concurrency Control"): the bare scalar `optimistic`, and nothing else. FEAT-2026-0088 (required key, `none` + `reason`) and FEAT-2026-0091 (closed `reason` vocabulary) are both **unshipped** as of generator 0.5.6.

`x-entity.concurrency: optimistic` opts an entity into optimistic locking. It is **optional** today and omitting it is the documented opt-out; `none` is *reserved* and errors. The richer semantics — required with no default, absence meaning *undeclared* rather than opted-out, and `{ mode: none, reason }` for a justified opt-out — arrive with FEAT-2026-0088, and are documented as a marked forward-looking block that authors are told not to write against yet.

**What the kit enforces:** the single accepted value, pinned by `schemas/spectral/fixtures/xentity-shape-keys.yaml` in both directions — `FullyDocumented` clean on `optimistic`; `BadConcurrencyMode`, `BareConcurrencyNone` and `ConcurrencyObjectForm` all flagged. The last two are the load-bearing cases: they are forms a *future* generator accepts and the pinned one errors on.

**A gap this exposed in the drift guard.** `check-extension-vocabulary.py` (§17) is one-way by design — fatal only when the generator knows a key the ruleset rejects. That asymmetry is justified for *keys*, because the generator reaches some through indirect constants. It does not extend to *values*: a ruleset that accepts a value the generator errors on is equally adoption-blocking, arrives with no warning, and the guard cannot see it. Kit `0.7.0` shipped that exact divergence in its first draft and it passed every check. Fixtures cover this instance; the general case is unsolved, and a value-level comparison is worth considering the next time the guard is touched.

**Three things the generator still owns:**

1. **The `reason` vocabulary is not frozen — and `reason` is not accepted at all yet.** A closed set is the right end state — the alternative is 80 near-identical sentences that cannot be audited in aggregate — and the consumer census that motivated it (86 entities) supports the collapse: administrative reference data ~23, append-only ~4, owner-only ~2. **The set must include a `not-assessed` member.** In that same census ~15 entities were genuinely contended and simply not analysed yet, which is a *status*, not a justification; a vocabulary of justifications cannot express it, and forcing those into `rare-write` or an `other` escape hatch manufactures exactly the false claims the vocabulary exists to make auditable. Proposed: `append-only | single-writer | reference-data | rare-write | not-assessed | other`, with `other` carrying free text and `not-assessed` the value FEAT-2026-0091 must refuse to promote past. The kit tightens its guard to whatever set the jar ships.
...
**Severity:** additive as a vocabulary. As a *finding*, it is not: a spec with no `concurrency` anywhere is not a protected spec, it is an unmeasured one, and the entities most likely to be missed are the human-vs-human contended ones (approval workflows, shared rosters) that an AI-safety framing does not surface. Note that until FEAT-2026-0088 ships there is no machine-readable way to distinguish "considered, needs no protection" from "never looked" — omission carries both, which is precisely the state the required-no-default design exists to end.

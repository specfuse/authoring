...
1. `x-references: <Target>` and a `belongsTo` naming the same target are **mutually exclusive for the same FK** — an *unbound* `belongsTo <Target>` and an `x-references: <Target>` on the same entity is an error. There is no precedence between them and no "degrades to a hint" fallback; if the entity is owned, use `belongsTo`, and if it merely points, use `x-references`. Enforced by the generator as **`DDD_DUAL_RELATIONSHIP_DECLARATION`** (error). Its message names both declarations and states that it refuses to pick silently; its suggested fix is the other half of rule 2 below — if the property *is* the composition's FK under a name the convention does not recognise, replace `x-references` with `x-fk-for: <Target>` rather than dropping either declaration.
2. **Binding exception.** A `belongsTo <Target>` is *consumed* by whichever property satisfies it — a conventionally-named `{target}Id`, or a property carrying `x-fk-for: <Target>` (§1.8). Once consumed, a **sibling** property may carry `x-references: <Target>` without conflict, because the two properties are different FKs expressing different relationships to the same entity. This mixed owning-plus-associating shape is legal and not uncommon: a `WorkItem` may `belongsTo Employee` through its owning FK while separately holding `reviewerEmployeeId` and `approvedByEmployeeId` as role associations. Rule 1 is about an unclaimed `belongsTo`, not about the target name appearing twice.
   ...
   The generator draws the same line, and it is worth knowing which side a shape falls on before authoring it. `DDD_DUAL_RELATIONSHIP_DECLARATION` fires on the property that *implements* the `belongsTo` and also declares `x-references` to that target — one edge cannot be both owned composition and non-owning association. It does **not** fire on sibling properties naming the same target, because those are different edges. Measured against the pinned `0.5.8` jar: `LegalEntity.organisationId` with both declarations errors, while `Associate.managerId` and `Associate.approverId` both pass on an `Associate` that declares `belongsTo: Person`. So **"points at another instance of my own owner's type" is expressed as an ordinary sibling `x-references`** — a self-referencing `Person.managerId -> Person` on an owned entity is legal, and wants a `description` saying which instance it points at.

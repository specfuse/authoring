...
| `groups[].generator` | string | optional | — | Internal — leave unset (or use `default`). See §7.4. |
...
| `groups[].artifacts[]` | array<string> | yes for code | — | Artifact types to emit (see §10). |
...
### 7.4 `generator` *(internal — leave unset)*
...
**Purpose**: List of artifact types the group emits. Each entry must match an artifact registered in the language's factory (§10).
...
### 7.11 `filter` *(AsyncAPI workers only, v1)*
...
## 9. Field Interactions and Cross-Field Behaviour
...
## 10. Supported Artifacts per Language
...
## 11. Deprecated and Silently-Ignored Fields
...
### 13.1 Minimal project (single C# Domain group)
...
### 13.2 Realistic project (multi-language, multi-group, AsyncAPI, Arazzo, tenancy)
...
5. **Per-group emission** — for each group: build the codegen configurator (stamping `projectName`, `dbContextClassnameOverride`, `hasEventRuntime`, every entry in `configs`, and the Dart `dartGroup.*` keys); run `cleanGenerated` if requested; invoke the artifact factory.
6. **Post-regen hooks** — once every group has written its tree, the `ChangelogTracker` snapshots each group's surface to `<stateDir>/<groupName>.json` and emits `CHANGELOG.md`. The Dart formatter then runs on every Dart/Flutter group's package root.
7. **`ai-access-manifest.json`** — written once in the parent directory of `openApiSpecifications`, not per-group.

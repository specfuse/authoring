...
| `registryEntryPath` | Path to the orchestration-repo registry entry, e.g., `<orchestrator>/features/FEAT-2026-0001.md`. Supplied by the caller — never assume a sibling layout. Referenced in §1 for traceability. |
...
`<orchestrator>` is supplied by the calling command, not assumed. Both files above are **authored per project** by the orchestration repo's onboarding agent — Specfuse ships neither — so "absent" and "wrong path" are different failures and must be reported as such.
